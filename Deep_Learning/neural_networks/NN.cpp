...
void NN::train_network(
    ...
    for (lint i = 1; i <= steps; ++i) {
        ...
        if (0 == i % epoch_size) {
            ...
            std::cout << "Epoch: " << epoch_size << '\n';
            std::cout << "loss: " << loss_sum / epoch_size << '\n';
            std::cout << "acc: " << accuracy_sum / epoch_size << "\n";
...
void NN::get_batch(lint batch_size,
    std::vector<std::vector<neurons::TMatrix<>>> & data_batch,
...
double NN::train_step(lint batch_size,
...
double NN::test_step(lint batch_size,
    const std::vector<std::vector<neurons::TMatrix<>>> & inputs,
    const std::vector<std::vector<neurons::TMatrix<>>> & targets,
    std::vector<std::vector<neurons::TMatrix<>>> & preds) {
    preds.resize(inputs.size());
    ...
    size_t thread_id = 0;
    for (; thread_id < new_threads; ++thread_id) {
        test_threads[thread_id] = std::thread([this, &inputs, &targets, thread_id, &preds] {
...
(lint batch_size, const std::vector<std::vector<neurons::TMatrix<>>>& inputs) const {
    ...
    size_t thread_id = 0;
    for (; thread_id < new_threads; ++thread_id) {
        test_threads[thread_id] = std::thread([this, &inputs, thread_id, &preds] {
...
double NN::get_accuracy(lint batch_size,
