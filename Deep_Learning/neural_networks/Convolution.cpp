...
neurons::Conv_1d::Conv_1d(const Shape & input_shape, const Shape & weights_shape, lint stride) : m_input_sh{ input_shape }, m_weights_sh{ weights_shape }, m_stride{ stride } {
...
neurons::TMatrix<> neurons::Conv_1d::operator()(const TMatrix<> & input, const TMatrix<> & weights, const TMatrix<> & bias) {
    if (this->m_input_sh.dim() < 3 || this->m_weights_sh.dim() < 3 || bias.m_shape.dim() < 2)    {
        throw std::invalid_argument(std::string("neurons::Conv_1d: Shape of inputs and weights should be compatible with the this convolution."));
    }
...
neurons::Conv_2d::Conv_2d(const Shape & input_shape,  const Shape & weights_shape, lint r_stride, lint c_stride, lint r_zero_p, lint c_zero_p)
    :
    ...
    if (input_shape.dim() != 4 || weights_shape.dim() != 4) {
        throw std::invalid_argument(
            std::string("neurons::Conv_2d: Shape needs to be compat"));
    }
    ...
    if (chls != this->m_weights_sh[2]) {
        throw std::invalid_argument(
            std::string("neurons::Conv_2d: Channels needs to compat"));
    }

    in_rows += 2 * m_r_zero_p;
    in_cols += 2 * m_c_zero_p;

    this->m_ex_in_sh = Shape{ in_batch_size, in_rows, in_cols } +  this->m_input_sh.sub_shape(3, this->m_input_sh.dim() - 1);
...
neurons::TMatrix<> neurons::Conv_2d::operator()(const TMatrix<> & input, const TMatrix<> & weights, const TMatrix<> & bias) {
    ...
    if (in_rows < w_rows || in_cols < w_cols) {
        throw std::invalid_argument(
            std::string("neurons::Conv_1d: SHape to compat with filter"));
    }
