...
std::string neurons::CNN_layer::from_binary_data(char * binary_data, lint & data_size, TMatrix<>& w, TMatrix<>& b, lint & stride, lint & padding,
...
neurons::CNN_layer::CNN_layer(double mmt_rate,  lint rows,  lint cols,  lint chls,  lint filters,
    lint filter_rows, lint filter_cols, lint stride, lint padding, lint threads, neurons::Activation *act_func, neurons::ErrorFunction *err_func)
    :
...
neurons::CNN_layer::CNN_layer(double mmt_rate, lint rows, lint cols, lint chls, lint stride, lint padding, lint threads, const TMatrix<>& w, const TMatrix<>& b, 
    std::unique_ptr<Activation>& act_func, std::unique_ptr<ErrorFunction>& err_func)
    :
    Traditional_NN_layer(mmt_rate, threads, w, b, act_func, err_func), m_conv2d{
...
std::vector<neurons::TMatrix<>> neurons::CNN_layer_op::batch_forward_propagate(const std::vector<TMatrix<>>& inputs, const std::vector<TMatrix<>>& targets) {
